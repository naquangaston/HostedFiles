...
`));
        ...
        //window.addEventListener('keydown', function (e) { const key = e.key; if(!!snipeM&&key=='f'){snipe()}})
        function fire(t, w) {
            setTimeout(function() {
                input.keyDown(32);
            }, t * 1000);
...
var autoSp=true
async function p1(){
    var [x,y]=[randomIntFromInterval(0,innerWidth),randomIntFromInterval(0,innerHeight)]
    input.mouse(x,y)
}
function p2(){
    p1()
...
snipe5M=true
        function snipe5(){
            for(let i=0;i<111;i++){
                setTimeout(()=>{input.mouse(mouse.x,mouse.y);},i)
            }
            input.keyDown(32);
            setTimeout(()=>{input.mouse(mouse.x,mouse.y)},10)
            setTimeout(()=>{input.mouse(mouse.x,mouse.y);input.keyUp(32)},100)
        }
        window.addEventListener('keydown', function (e) { const key = e.key; if(!!snipe5M&&key=='r'){snipe5()}})`);didd_=true}
