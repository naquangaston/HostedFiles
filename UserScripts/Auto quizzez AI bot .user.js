...
setValue = GM_setValue, getValue = GM_getValue, info_ = GM_info, "quizizz.com" == document.domain && function() {
    ...
    let y, E = 0;
    var x = function() {
        b(), console.log("Updated choices"), [...h].forEach(((e, t) => {
            var n = e.addEventListener("click", (function() {
                v++, console.log("Added"), e.removeEventListener(n), r.apply(this, [t, ...arguments])
            }))
        })), console.log("Updated score"), Number(document.getElementsByClassName("score-title")[0].innerText), clearTimeout(y), f && (console.log("Getting answer"), y = setTimeout(o, 3e3))
    };
    let N = "";
    setInterval((() => {
        try {
            let e = t() + "; " + u;
            N != e && (console.clear(), f = 1, p = 0, x()), N = e
        } catch (e) {}
    })), setInterval((() => {
        for (let e in w)
            if (!w[e].disabled) {
