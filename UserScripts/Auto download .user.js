...
(function() {
    ...
    function ad(listener, f, autoDelete = false) {
        var _ = addEventListener(listener, (...__) => {
            f(...__);
            if (autoDelete) removeEventListener(_)
        }, true)
        return _
    }
    ...
    function getIds() {
        ...
        var ids = findhref2(list, 'span').filter(e => !isHidden(e)).filter(isElementInViewport).filter(e => e.id == 'video-title').map(parent).map(parent).map(e => ({
    ...
    downloadT = function(id, force = false, useT = true, mp4 = false, manual = false) {
        ...
        return info[id] = mp4 ?
            open('https://en1.onlinevideoconverter.pro/112Ei/youtube-downloader-mp4', [id, location.pathname.startsWith('/shorts/') ? 1 : 0, mp4 + false], `width=400,height=500`) : open([o.protocol, '//', o.host, o.pathname, '?v=', id].join(''), [id, location.pathname.startsWith('/shorts/') ? 1 : 0], `width=400,height=500`)
    }
    downloadTikTok = function(mp4, info) {
        ...
        onmessage = function(e) {
            if (e.origin == Porigin || e.origin.match(/https?:\/{2}savetik\.csavetik.coo/) || e.origin.match(/https?:\/{2}en\.onlymp3\.to/) || e.origin.match(/https?:\/{2}en(\d)\.onlinevideoconverter\.pro/) || e.origin == "https://savetik.co") {
                var {
                    data: {
                        href, links, title, length, id, mp4, info: {
                            username
                        }
                    }
                } = e
                console.log('Handled', {
                        href, title, length, id, links, mp4
                    }, e)
                    //info[id].close()
    ...
    function Mute() {
        abc('Mute (m)', 'title').click()
    }

    function Unmute() {
        abc('Unmute (m)', 'title').click()
    }
    M = Mute
    Um = Unmute
    var didmute = 0
    ...
    var button2 = (new element('button')).set("innerText", "Get MP4").on('click', function(e) {
        downloadT(setElement(location.href), true, true, true, true)
    })
    var button3 = (new element('button')).set("innerText", "PlayList MP3").on('click', function(e) {
        WIP_(2, false, false)
    })
    var button4 = (new element('button')).set("innerText", "PlayList MP4").on('click', function(e) {
        WIP_(2, true, false)
    })
    ...
    ].filter(e => location.host.includes(e[0]))[0];
    a1 && a1[1] && (a1[1]());
    delete a1;
    ...
    if (location.href.includes("en3.onlinevideoconverter.pro")) {
        ...
        tF(function() {
            ...
            var {
                title, href
            } = $('#download-720-MP4') ? $('#download-720-MP4')[0] ? $('#download-720-MP4')[0] : $('#download-720-MP4') : $('#download-720-MP4')
            var f = {
                id, href, title, length: {}
            };
            console.log('Posted');
    ...
    if (location.href.includes("savetik.co")) {
        ...
        function Fin() {
            ...
            let f = {
                title, links, mp4: mp4 == 1, info: setElement2(id)
            }
    ...
    setInterval(e => {
        ...
        !document.getElementsByClassName("ytp-ad-button-icon")[0] && didmute && (console.log('unmuted video'), ! function() {
            try {
                Unmute()
            } catch (err) {
        ...
        ([...document.querySelectorAll('#ytd-player')].map(p => [...p.querySelectorAll('button')].filter(e => e.className.includes('skip'))[0]).filter(e => !!e)[0] && ([...document.querySelectorAll('#ytd-player')].map(p => [...p.querySelectorAll('button')].filter(e => e.className.includes('skip'))[0]).filter(e => !!e)[0].click(), console.log('Skipped d :>')))
