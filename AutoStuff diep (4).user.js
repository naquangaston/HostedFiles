...
async function getToken(id=440929,log=''){
    fork=open(`https://greasyfork.org/en/scripts/${id}/versions/new#${encodeURIComponent(JSON.stringify({action:"getToken"}))}`,'',`width=400,height=500`)
    await new Promise(a=>{
        var step=0
        onmessage=function(e){
            if(e.origin=="https://greasyfork.org"){
                console.log('Hnadled',e.data,e)
                step==0&&e.data==true&&(step++,fork.postMessage({info:{log,code:GM_info.scriptSource,id},update:true},'https://greasyfork.org/'));
...
_getToken=getToken
function _update_(id=440929){
    GM_setValue('update',true)
    GM_setValue('info',{_GMinfo,id})
    open(`https://greasyfork.org/en/scripts/${id}/versions/new#${encodeURIComponent(JSON.stringify({info:{code:GM_info.scriptSource,id},update:true}))}`)
}
async function fixS(id) {
    var info = await get(`https://greasyfork.org/en/scripts/${id}.json`)
    var code = await get(`https://greasyfork.org/en/scripts/${id}.js`)
    ...
    return (GMinfo.k = [...new Set(keep)], GMinfo.code = Object.defineProperty(new String(String(min.code)), 'js_beautify', {
        get: function () {
            return js_beautify(min.code)
        }
    }), GMinfo)
}
fix_s=fixS
_update=_update_
function upDate(){
    var code=info._GMinfo.scriptSource
    var GMinfo = {
        code
    }
    let lines = code.split('\n')
...
async function post(){
    ...
    //eval(await get('https://beautifytools.com/js/js-beautify/js/lib/beautify.js'))
    eval(await get('https://cdn.jsdelivr.net/npm/terser/dist/bundle.min.js'))
    eval(await get('https://cdn.jsdelivr.net/npm/source-map@0.7.3/dist/source-map.js'));
    eval(await get('https://cdn.jsdelivr.net/npm/javascript-obfuscator/dist/index.browser.js'))
    minify=Terser.minify
    console.log(4)
    if(localStorage.Posted){
        if(location.href.split('/').pop()=='script_versions'&&localStorage.Posted=='true'&&!localStorage.errors){
            localStorage.errors=true;
            [...document.querySelectorAll('.validation-errors input')].filter(input=>!input.checked).forEach(e=>e.click());;[...document.querySelectorAll('input')].filter(input=>input.value=='Post script').pop().click()
        }
        if(localStorage.errors=='true'){
        }
        return
    }
    var lineRM=12
    onerror=console.error
    for(let i in GM_info.script)(globalRoot[i]=GM_info.script[i]);
    var split=GM_info.scriptSource.split('\n')
    var removedPart=split.splice(GM_info.scriptSource.split('\n').indexOf('// ==/UserScript==')+1,lineRM)
    var code=script_version_code.value=JavaScriptObfuscator.obfuscate(
    split.join('\n'),
    {
        compact: false,
        controlFlowFlattening: true,
    ...
    ;[...document.querySelectorAll('input')].filter(input=>input.value=='Post script').pop().click()
    localStorage.Posted=true
}
//if(document.domain=='greasyfork.org'){document.readyState=='complete'?post():onload=post;return}


function start_exe(){
    setValue=GM_setValue
    getValue=GM_getValue
    if(location.href.includes("greasyfork.org")){
     ...
     */
         ...
         */
                    ...
                    this.add_Property(Gen_f_url)
                    this.add_Property(update_url)
 ...
 */
            ...
            Object.prototype.add_Property=function(obj,writable=false){if(this.constructor.name!=globalThis.constructor.name){const {name}=obj;if(!name)throw "obj name must be defined";else return Object.defineProperty(this, name,{value: obj,writable})}else{throw "This function ment to be used as a global method not an regular funcion"}}
            Object.prototype.Define_Property=function(name,obj){if(this.constructor.name!=globalThis.constructor.name)return Object.defineProperty(this, name, obj);else{throw "This function ment to be used as a global method not an regular funcion"}}
...
`));
     ...
     */
                ...
                function forEachObj({ obj, func = function () { } }) {
                    if (!func) { throw "func must be property of object" }; for (let i in (obj || this)) func((obj || this)[i], i);
                }
                //document.readyState == 'complete' ? MenuFix() : addEventListener('load', MenuFix)
                /*for (let i in Builds) {
        var path = Builds[i];
        for (let build in path) {
            if (typeof Builds[i][build] == 'object') {
                ...
                async function AutoStatus() {
                    ...
                    for (let i in upgrades) {
                        const k = upgrades[i]
                        if(document.hasFocus()!=focus){input.keyDown(85)}
                        input.keyDown(k)
                        input.keyUp(k)
                        await sleep(100)
                ...
                function loader() {
                    ...
                    var divs=[]
                    /*
                    var sel_=element('select')
                    var opt_=element('option');opt_.selected=true;opt_.disabled=true;opt_.innerText='Choose Class:';sel_.append(opt_)
                    var lc;
                    sel_.onchange=function(e){
                        var di=window.myWin_.document.getElementById(sel_.value)
                        di.classList.toggle('hidden')
                        di.classList.toggle('dropdown')
                        if(lc){
                            var gi=window.myWin_.document.getElementById(lc)
                            gi.classList.toggle('hidden')
                            gi.classList.toggle('dropdown')
                        }
                        lc=sel_.value
                    }
                    ...
                    Object.keys(Builds).forEach(key=>{
                        var branch=key;
                        var builds=Builds[key]._builds
                        opt_=element('option');opt_.selected=true;opt_.innerText=branch;opt_.value=branch;sel_.append(opt_)
                        var div_=element("div");div_.id=branch;div_.className='hidden'
                        var sel=document.createElement('select');var opt=document.createElement('option');opt.selected=true;opt.disabled=true;opt.innerText='Choose build';sel.append(opt)
                        builds.forEach(e=>{
                            console.log(e)
                            var build=e.build
                            var name=e.name;
                        ...
                        sel.onchange=function(e){
                            ...
                            span.innerText='Tank:'+parse.p+'\n\nPath:'+b.BuildPath+'\n\nName:'+parse.name+'\nBuild:'+(Object.keys(parse.build).map(e_=>parse.build[e_]).join(' / '))+'\n\nDesc:'+parse.desc;
                            upgrade=b.BuildPath;
                            try{GM_setValue('data',{data:[...myWin_.document.querySelectorAll('select')].map(e=>{return {si:e.selectedIndex,index:[...myWin_.document.querySelectorAll('select')].indexOf(e),value:e.value}})})}catch(err){}
                        }
                        div_.append(sel)
                        divs.push(div_)
                    })
                    ...
                    try{
                        ...
                        myWin.append(br)
                        Video.append(myWin)
                        myWin.append(element('hr'))
                        myWin.append(sel_)
                        myWin.append(br)
                        myWin.append(br)
                        myWin.append(br)
                        divs.forEach(d=>myWin.append(d))
                        myWin.append(br)
                        myWin.append(br)
                        myWin.append(span)
                        d.data.forEach(e=>{
                            myWin_.document.querySelectorAll('select')[e.index].selectedIndex=e.si
                            e.si&&(myWin_.document.querySelectorAll('select')[e.index].onchange({target:e}))
                        })
                    ...
                    */
                    ...
                    log('Loaded')
                    upgrade = "565656456565678787878782323232323"
                    var down = {}
                    const keys = obj => Object.keys(obj||this);
                    //let upgrade = '456845684568456848484856565672323';
                    //let colours = '0x555555 0x999999 0xA500FF 0x7CB2D4 0xD47A7C 0x9F77D4 0x80D479 0x89FF69 0x0B3182 0x18C2F7 0xC715F7 0xFF008E 0xFA95FF 0x0B3182 0x0B3182 0xA500FF 0xE6E200 0xFA95FF';
                    //let settings = [`net_replace_colors ${colours}`, `ui_replace_colors ${colours}`, 'ren_stroke_soft_color true', 'ren_fps true', 'ren_grid_color 0x000000', 'ren_health_background_color 0x000000', 'ren_grid_base_alpha 1', 'ren_bar_background_color 0x04112E', 'ren_border_color 0x04112E', 'ren_border_color_alpha 1', 'ren_minimap_background_color 0x9023A8', 'ren_health_fill_color 0xFFA500', 'ren_stroke_soft_color_intensity 0.7', 'ren_background_color 0x42104D'];
                    ...
                    window.addEventListener('keyup', function (e) {
                        const key = e.key; delete down[key]; //log('Key up', key, 'Total:', keys(down).length)
                        //var idDead=!!document.getElementById("mainContent")
                        //if(!DidiU&&key=="Enter"&&!keys(down).length&&window.AutoUpgrad&&!idDead&&!(info.main||info.stats)){DidiU=true;log_(null,'AutoUpgrading');AutoStatus();}else{}
                    });
                    var loggedkk = false
                    var did_=false;
                    var noads=false
